...
// Initialize
mi_decl_internal void _mi_prim_mem_init( mi_os_mem_config_t* config );

// Free OS memory
mi_decl_internal int _mi_prim_free(void* addr, size_t size );
...
mi_decl_internal int _mi_prim_alloc(void* hint_addr, size_t size, size_t try_alignment, bool commit, bool allow_large, bool* is_large, bool* is_zero, void** addr);

// Commit memory. Returns error code or 0 on success.
// For example, on Linux this would make the memory PROT_READ|PROT_WRITE.
// `is_zero` is set to true if the memory was zero initialized (e.g. on Windows)
mi_decl_internal int _mi_prim_commit(void* addr, size_t size, bool* is_zero);

// Decommit memory. Returns error code or 0 on success. The `needs_recommit` result is true
// if the memory would need to be re-committed. For example, on Windows this is always true,
// but on Linux we could use MADV_DONTNEED to decommit which does not need a recommit.
// pre: needs_recommit != NULL
mi_decl_internal int _mi_prim_decommit(void* addr, size_t size, bool* needs_recommit);

// Reset memory. The range keeps being accessible but the content might be reset to zero at any moment.
// Returns error code or 0 on success.
mi_decl_internal int _mi_prim_reset(void* addr, size_t size);
...
mi_decl_internal int _mi_prim_reuse(void* addr, size_t size);

// Protect memory. Returns error code or 0 on success.
mi_decl_internal int _mi_prim_protect(void* addr, size_t size, bool protect);

// Allocate huge (1GiB) pages possibly associated with a NUMA node.
// `is_zero` is set to true if the memory was zero initialized (as on most OS's)
// pre: size > 0  and a multiple of 1GiB.
//      numa_node is either negative (don't care), or a numa node number.
mi_decl_internal int _mi_prim_alloc_huge_os_pages(void* hint_addr, size_t size, int numa_node, bool* is_zero, void** addr);

// Return the current NUMA node
mi_decl_internal size_t _mi_prim_numa_node(void);

// Return the number of logical NUMA nodes
mi_decl_internal size_t _mi_prim_numa_node_count(void);

// Clock ticks
mi_decl_internal mi_msecs_t _mi_prim_clock_now(void);
...
mi_decl_internal void _mi_prim_process_info(mi_process_info_t* pinfo);

// Default stderr output. (only for warnings etc. with verbose enabled)
// msg != NULL && _mi_strlen(msg) > 0
mi_decl_internal void _mi_prim_out_stderr( const char* msg );

// Get an environment variable. (only for options)
// name != NULL, result != NULL, result_size >= 64
mi_decl_internal bool _mi_prim_getenv(const char* name, char* result, size_t result_size);


// Fill a buffer with strong randomness; return `false` on error or if
// there is no strong randomization available.
mi_decl_internal bool _mi_prim_random_buf(void* buf, size_t buf_len);

// Called on the first thread start, and should ensure `_mi_thread_done` is called on thread termination.
mi_decl_internal void _mi_prim_thread_init_auto_done(void);

// Called on process exit and may take action to clean up resources associated with the thread auto done.
mi_decl_internal void _mi_prim_thread_done_auto_done(void);

// Called when the default heap for a thread changes
mi_decl_internal void _mi_prim_thread_associate_default_heap(mi_heap_t* heap);
